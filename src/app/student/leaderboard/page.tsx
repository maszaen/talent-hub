  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Peringkat 100 besar mahasiswa berdasarkan perolehan poin tertinggi.</p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Hall of Fame</CardTitle>
          <CardDescription>Peringkat dihitung berdasarkan total poin yang telah diverifikasi admin.</CardDescription>
        </CardHeader>
            ...
            <TableBody>
              {students.length === 0 ? (
                ...
                students.map((student, index) => {
                  ...
                  return (
                    <TableRow key={student.id} className={`${isMe ? 'bg-primary/10 hover:bg-primary/20' : index < 3 ? 'bg-muted/50' : ''}`}>
                      ...
                      <TableCell>
                        <div className="font-medium flex items-center gap-2">
                          {student.name}
                          {isMe && <Badge variant="default" className="text-[10px] h-5">Kamu</Badge>}
                        </div>
                      </TableCell>
